...
exec "$JAVA" $JAVA_OPTS $MAXHEAP $JAVA_OPTS_SCRIPT $ORIENTDB_SETTINGS $MAXDISKCACHE -Djava.util.logging.config.file="$LOG_FILE" -Dorientdb.config.file="$CONFIG_FILE" -Dorientdb.www.path="$WWW_PATH" -Dorientdb.build.number="UNKNOWN@r${buildNumber}; 2015-08-05 13:38:23+0000" -cp "$ORIENTDB_HOME/lib/orientdb-server-2.1.0.jar:$ORIENTDB_HOME/lib/*" $* com.orientechnologies.orient.server.OServerMain
