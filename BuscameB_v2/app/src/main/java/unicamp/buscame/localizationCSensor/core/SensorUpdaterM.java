...
import unicamp.buscame.localizationCSensor.req.ILocalizationManager;
import unicamp.buscame.localizationCSensor.req.ISensorUpdater;
        ...
        this.turnOn = true;
            while (this.turnOn) {
                try {
                    ...
                    Log.d("Buscame", "Error = Localization C Sensor " + e.toString());
                }
            }
    ...
    private synchronized void executeSensor() {
        ...
        //performance true and false
            ...
            if(totalTime>100000){
                ...
                Log.d("Buscame", "Localization C Sensor(Activated)I: "+timeSensorI+" " + nome + " Time: " + totalTime + " ");

                sensorUpdater.activateSensor(nome);

                long timeSensorF= System.currentTimeMillis();
                Log.d("Buscame", "Localization C Sensor(Activated)F: " + timeSensorF + " " + nome + " Time: " + totalTime + " ");
            }else{
                long timeSensorI= System.currentTimeMillis();
                android.util.Log.i("Buscame", "Localization C Sensor(Deactivated)I: " + timeSensorI + " ");

                sensorUpdater.deactivateSensor(nome);

                long timeSensorF= System.currentTimeMillis();
                android.util.Log.i("Buscame", "Localization C Sensor(Deactivated)F: " + timeSensorF + " ");
            }
        }catch (Exception e){
